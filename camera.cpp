...
void Camera::Write(int x, int y, std::string_view text, COLORREF color, HFONT font){
    static wchar_t wbuf[256];
    int wlen = MultiByteToWideChar(CP_ACP, 0, text.data(), (int)text.size(), wbuf, 255);
...
void Camera::DrawFrame(PixelBuffer& pb){
    ...
    //if(tile.faces[i])
    //for(int j = 0; j < 2; j++)
    //Project(vecTrianglesToRaster, unityCube.m.tris[2*i+j], tile.position, tile.angle, tile.color);

    {
        ...
        for(size_t i = 0; i < numThreads; i++){
            ...
            threads.emplace_back([&, i, start, end](){
                ...
                if(tiles[j].faces[k])
                for(int w = 0; w < 2; w++){
                    //std::cout << (std::string)unityCube.m[2*k+w]+"cube\n";
                    Project(threadBuckets[i], unityCube.m[2*k+w], tiles[j].position, tiles[j].angle, tiles[j].color);
                }
            });
        }
    ...
    //Project(vecTrianglesToRaster, t, {0, 0, 0}, {0, 0, 0}, t.color);
...
void Camera::DrawFrameGL(){
    ...
    //glClearColor(
    //    GetRValue(BackGround) / 255.0f, 
    //    GetGValue(BackGround) / 255.0f, 
    //    GetBValue(BackGround) / 255.0f, 
    //    1.0f
    //);
    ...
    for(Tile& tile : tiles){
        ...
        if(tile.faces[i])
        for(int j = 0; j < 2; j++){
            //vec3D normal = unityCube.m[2*i+j].normal(); //((unityCube.m[2*i+j].p[1] - unityCube.m[2*i+j].p[0])%(unityCube.m[2*i+j].p[2] - unityCube.m[2*i+j].p[0])).normalize();
            //glNormal3f(normal.x, normal.y, normal.z);
            glNormal3f(unityCube.m[2*i+j].n.x, unityCube.m[2*i+j].n.y, unityCube.m[2*i+j].n.z);
            ...
            glVertex3f(unityCube.m[2*i+j].p[0].x + tile.position.x, unityCube.m[2*i+j].p[0].y + tile.position.y, unityCube.m[2*i+j].p[0].z + tile.position.z);
            glVertex3f(unityCube.m[2*i+j].p[1].x + tile.position.x, unityCube.m[2*i+j].p[1].y + tile.position.y, unityCube.m[2*i+j].p[1].z + tile.position.z);
            glVertex3f(unityCube.m[2*i+j].p[2].x + tile.position.x, unityCube.m[2*i+j].p[2].y + tile.position.y, unityCube.m[2*i+j].p[2].z + tile.position.z);
        }
    }
    for(mesh& mesh : meshes){
        for(triangle& t : mesh.tris){
            //vec3D normal = t.normal(); //((t.p[1] - t.p[0])%(t.p[2] - t.p[0])).normalize();
            //glNormal3f(normal.x, normal.y, normal.z);
...
void Camera::Project(std::vector<triangle> &vecTrianglesToRaster, triangle& tri, vec3D pos, vec3D ang, COLORREF color){
    triangle triWorld = tri.rotate(ang) + pos; 
    triangle triViewed = triWorld * matView;

    //if(triViewed.normal() * triViewed.p[0] < 0.0f){
    if(triViewed.n * triViewed.p[0] < 0.0f){
        ...
        for (int n = 0; n < nClippedTriangles; n++){
            ...
            if(triProjected.color != CLR_INVALID) triProjected.color *= vec3D{dp, dp, dp};
            vecTrianglesToRaster.push_back(triProjected);
        }
    }
}

/*void Camera::Project(std::vector<triangle> &vecTrianglesToRaster, mesh mesh, vec3D pos, vec3D ang){
...
void Camera::Project(std::vector<triangle> &vecTrianglesToRaster, Tile tile){   
    for(int i = 0; i < 6; i++)
    if(tile.faces[i])
    for(int j = 0; j < 2; j++){
        triangle triWorld = unityCube.m.tris[2*i+j].rotate(tile.angle) + tile.position; 
        triangle triViewed = triWorld * matView;
        ...
        if(triViewed.normal()*triViewed.p[0] < 0.0f){
            ...
            for(int n = 0; n < nClippedTriangles; n++){
                triangle triProjected = clipped[n] * matProj + 1.0f;
                triProjected *= vec3D{(float)screenWidth/2, (float)screenHeight/2, 1};
